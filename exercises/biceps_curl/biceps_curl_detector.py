"""
Module de détection et comptage des biceps curls.

Ce module utilise MediaPipe Pose pour détecter les biceps curls en temps réel
en analysant l'angle du coude.
"""
...
from utils.angle_calculator import calculate_elbow_angle


class BicepsCurlDetector:
    """
    Détecteur de biceps curls utilisant MediaPipe Pose.
    
    Analyse l'angle du coude pour détecter les répétitions de biceps curls.
    """
    
    def __init__(self, config_path: Optional[str] = None):
        """
        Initialise le détecteur de biceps curls.
        ...
        if config_path is None:
            config_path = Path(__file__).parent / "biceps_curl_config.json"
        ...
        self.min_angle = self.config.get('min_angle', 50.0)  # Angle minimum (position basse)
        ...
        # État de détection
        self.counter = 0
        self.stage = "down"  # "down" ou "up"
        self.current_angle = None
        
    def detect(self, frame: np.ndarray) -> Tuple[np.ndarray, Dict]:
        """
        Détecte les biceps curls dans une frame vidéo.
        ...
        if results.pose_landmarks:
            ...
            self.current_angle = calculate_elbow_angle(
                results.pose_landmarks,
                side=self.side
            )
    ...
    def _update_counter(self) -> None:
        """
        Met à jour le compteur de répétitions basé sur l'angle du coude.
        """
        if self.current_angle is None:
            return
        
        # Position basse: angle < min_angle
        if self.current_angle < self.min_angle:
            if self.stage == "up":
                self.stage = "down"
        
        # Position haute: angle > max_angle
